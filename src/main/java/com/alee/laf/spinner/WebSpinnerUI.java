...
public class WebSpinnerUI extends BasicSpinnerUI implements ShapeProvider, BorderMethods
{
    ...
    protected void replaceEditor ( final JComponent oldEditor, final JComponent newEditor )
    {
        ...
        updateEditorStyle ( newEditor );
    }

    protected void updateEditorAlignment ( final JComponent editor )
    {
        if ( editor instanceof JSpinner.DefaultEditor )
        {
            // if editor alignment isn't set in LAF, we get 0 (CENTER) here
            final int alignment = UIManager.getInt ( "Spinner.editorAlignment" );
            final JTextField text = ( ( JSpinner.DefaultEditor ) editor ).getTextField ();
            text.setHorizontalAlignment ( alignment );
        }
    }

    protected void updateEditorStyle ( final JComponent editor )
    {
    ...
    public static void installFieldUI ( final JTextComponent field, final JSpinner spinner )
    {
        ...
        textFieldUI.setFieldMargin ( new Insets ( 2, 2, 2, 2 ) );
