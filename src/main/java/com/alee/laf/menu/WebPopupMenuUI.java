...
public class WebPopupMenuUI extends BasicPopupMenuUI implements SwingConstants, Styleable, ShapeProvider, BorderMethods
{
            ...
            public void propertyChange ( final PropertyChangeEvent evt )
            {
                ...
                if ( evt.getNewValue () == Boolean.TRUE )
                {
                    ...
                    if ( !SystemUtils.isUnix () )
                    {
                        ...
                        installPopupSettings ( SwingUtils.getWindowAncestor ( popupMenu ), popupMenu );
                    }
                }
                else
                {
                    // Uninstall custom popup window settings
                    uninstallPopupSettings ( SwingUtils.getWindowAncestor ( popupMenu ), popupMenu );
                }
            }
        };
     ...
     */
    protected void installPopupSettings ( final Window window, final JPopupMenu popupMenu )
    {
        if ( window != null && transparent && SwingUtils.isHeavyWeightWindow ( window ) )
        {
            ...
            if ( SwingUtils.isHeavyWeightWindow ( window ) )
            {
                ...
                else if ( ProprietaryUtils.isWindowShapeAllowed () )
                {
                    ...
                    ++bounds.width;
                    ++bounds.height;
                    final Shape shape = ( ( WebPopupPainter ) painter ).provideShape ( popupMenu, bounds );
     ...
     */
    @SuppressWarnings ("UnusedParameters")
    protected void uninstallPopupSettings ( final Window window, final JPopupMenu popupMenu )
    {
        if ( window != null && transparent && SwingUtils.isHeavyWeightWindow ( window ) )
        {
    ...
    public Popup getPopup ( final JPopupMenu popup, int x, int y )
    {
        ...
        final Component window = ReflectUtils.callMethodSafely ( p, "getComponent" );
